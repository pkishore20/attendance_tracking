...
class SearchController extends Controller
{
    ...
    public function search_user(Request $request)
    {
        try {    
            ...
            if ($request->has('date')) {
                $date_ids = attendance::where('date', $request->date)->get()->pluck('users_info_id')->toArray();
            
                if (in_array($user_id, $date_ids)) {
                    $data = $query->where('id', $user_id)->get();
                } else {
                    return $this->sendErrorResponse('No data records for this user attendance .');
                }
            }
                if ($data->isEmpty()) {
                    return $this->sendErrorResponse('No results found.');
                 ...
                 $data1 = [];
                    foreach ($data as $item) {
                         $data1[] = [
                             ...
                             'in_time'       => $item->attendance->in_time,
                             'out_time'      => $item->attendance->out_time,
                             'total_hours'   => diff_time($item->attendance->in_time, $item->attendance->out_time),
                        ];
                    }
            return $this->sendSuccessResponse('Search results retrieved successfully', $data1);
